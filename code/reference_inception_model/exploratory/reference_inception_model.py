...
"""
Reference Inception-style CNN model with tuned hyperparameters.

This script mirrors the reference CNN, but replaces the Conv1D stack with
an inception-style block that uses multiple kernel sizes in parallel
([3, 5, 7, 9, 13]). It trains on the preprocessed reference_model data,
then reports test MSE, MAE, and R², and saves outputs to
`/mnt/oak/users/tami/5utr_cnn/output/inception_model`.
"""
...
from reference_model.config import updated_params_4, NUM_EPOCH, DATA_DIR, OUTPUT_DIR
...
def main():
    # Output directory for inception model artifacts
    inception_dir = os.path.join(OUTPUT_DIR, "inception_model")
    ...
    print("Using tuned parameters (updated_params_4):")
    ...
    # Train with early stopping (max 150 epochs)
    ...
    history = model.fit(
        ...
        epochs=150,  # Max epochs (early stopping will stop earlier if no improvement)
    ...
    plt.ylabel("MSE Loss")
    plt.ylim((0, 3))
    plt.title(f"Inception CNN: Training and Validation Loss (R²={r2:.3f})")
    plt.legend()
