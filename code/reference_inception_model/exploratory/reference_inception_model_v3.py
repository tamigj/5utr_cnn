...
"""
Reference Inception-style CNN model - v3 with expanded filter sizes.

Improvements over v2:
  - More filter sizes: [3, 5, 7, 9, 11, 13, 15] (7 branches vs 5)
  - 16 filters per branch = 112 total filters (vs 64 in v2)
  - Same good regularization: l2=0.1, lr=0.0005
  - Same architecture improvements: ReduceLROnPlateau, wider dense layers

Outputs saved to `/mnt/oak/users/tami/5utr_cnn/output/inception_model_v3`.
"""
...
def build_inception_cnn_model(
    ...
    Differences from the baseline CNN:
      ...
      - Each branch has 16 filters (total 112 filters per inception block).
    """
    ...
    n_filters_per_branch = 16  # Fixed: 16 filters per branch
...
def main():
    # Output directory for v3 inception model artifacts
    inception_dir = os.path.join(OUTPUT_DIR, "inception_model_v3")
    ...
    params["n_filters"] = 112  # 16 filters per branch * 7 branches = 112 total
    ...
    print(f"  n_filters: {params['n_filters']} (16 per branch * 7 branches = 112 total)")
    print(f"  n_dense_units: {params['n_dense_units']} (was {updated_params_4['n_dense_units']})")
    print(f"  filter_sizes: [3, 5, 7, 9, 11, 13, 15] (7 branches)")
    ...
    plt.ylabel("MSE Loss")
    plt.ylim((0, 3))
    plt.title(f"Inception CNN (v3): Training and Validation Loss (R²={r2:.3f})")
    plt.legend()
