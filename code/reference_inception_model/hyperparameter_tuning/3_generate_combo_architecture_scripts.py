#!/usr/bin/env python
"""Generate and submit tuning scripts for combinatorial architecture search.

This script creates bash submit scripts for each combination of architecture
parameters in combinatorial_tuning_grid_architecture and submits them to SLURM.

Usage:
    python 3_generate_combo_architecture_scripts.py
"""
...
param_names = list(combinatorial_tuning_grid_architecture.keys())
param_value_lists = list(combinatorial_tuning_grid_architecture.values())
combinations = list(itertools.product(*param_value_lists))
n_combinations = len(combinations)

print("=" * 70)
print("Generating combinatorial architecture tuning scripts...")
print("=" * 70)
...
for combo_idx, combo in enumerate(combinations, 1):
    ...
    primary_output_dir = os.path.join(
        OUTPUT_COMBO_ARCHITECTURE_DIR, f"{primary_param}_{primary_value_id}"
    )
    ...
    global_summary_csv = os.path.join(OUTPUT_COMBO_ARCHITECTURE_DIR, "summary.csv")
    ...
    script_name = f"submit_combo_architecture_{combo_id}.sh"
...
#SBATCH --job-name=combo_arch_{combo_id}
#SBATCH --output={logs_dir}/combo_architecture_{combo_id}_%j.log
#SBATCH --error={logs_dir}/combo_architecture_{combo_id}_%j.err
...
"""
    ...
    if result.returncode == 0:
        ...
        print(
            f"[{combo_idx}/{n_combinations}] ✓ Generated and submitted {script_name} (Job ID: {job_id})"
        )
        print(f"    Parameters: {combo_params}")
        total_scripts += 1
    else:
        print(
            f"[{combo_idx}/{n_combinations}] ✗ Error submitting {script_name}: {result.stderr}"
        )

print(f"\n{'=' * 70}")
print(f"Generated and submitted {total_scripts} combinatorial architecture tuning scripts")
print(f"Output directory: {OUTPUT_COMBO_ARCHITECTURE_DIR}")
