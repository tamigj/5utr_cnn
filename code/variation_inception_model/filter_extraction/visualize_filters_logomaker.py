...
Outputs are saved separately for REF and ALT branches.
"""
...
OUTPUT_DIR_FILTERS_REF = os.path.join(OUTPUT_DIR, 'variation_inception_model', 'filter_extraction', 'siamese_ref', 'extracted_filters')
OUTPUT_DIR_FILTERS_ALT = os.path.join(OUTPUT_DIR, 'variation_inception_model', 'filter_extraction', 'siamese_alt', 'extracted_filters')
OUTPUT_DIR_VIS_REF = os.path.join(OUTPUT_DIR, 'variation_inception_model', 'filter_extraction', 'siamese_ref', 'visualizations_logomaker')
OUTPUT_DIR_VIS_ALT = os.path.join(OUTPUT_DIR, 'variation_inception_model', 'filter_extraction', 'siamese_alt', 'visualizations_logomaker')
os.makedirs(OUTPUT_DIR_VIS_REF, exist_ok=True)
os.makedirs(OUTPUT_DIR_VIS_ALT, exist_ok=True)
...
def weights_to_contribution_matrix(filter_weights):
    ...
    Args:
        filter_weights: Array of shape (filter_size, 4) with raw weights
                       Order: [A, C, G, T] based on one-hot encoding
...
def create_combined_logomaker_figure(filters_by_size, output_dir, branch_name):
    ...
    for filter_size, filters in filters_by_size.items():
        ...
        plt.suptitle(f'All Filters (size {filter_size}) - {branch_name} Branch - Logomaker', fontsize=16, y=0.995)
        plt.tight_layout()
...
def visualize_filters_for_branch(branch_name, output_dir_filters, output_dir_vis):
    """Visualize filters for a specific branch (REF or ALT)."""
    print(f"\n{'='*70}")
    print(f"Visualizing Filters with Logomaker ({branch_name} Branch)")
    print(f"{'='*70}")
    ...
    print(f"\nCreating individual logomaker logos ({branch_name} branch)...")
    for filter_size, filters in filters_by_size.items():
        n_filters = filters.shape[0]
    ...
    print(f"\nCreating combined figures ({branch_name} branch)...")
    create_combined_logomaker_figure(filters_by_size, output_dir_vis, branch_name)
    
    print(f"\n{'='*70}")
    print(f"Logomaker Visualization Complete ({branch_name} Branch)!")
    print(f"{'='*70}")
...
def main():
    print("=" * 70)
    print("Visualizing Filters with Logomaker (Siamese Model)")
    print("=" * 70)
    
    # Visualize REF branch
    visualize_filters_for_branch("REF", OUTPUT_DIR_FILTERS_REF, OUTPUT_DIR_VIS_REF)
    
    # Visualize ALT branch
    visualize_filters_for_branch("ALT", OUTPUT_DIR_FILTERS_ALT, OUTPUT_DIR_VIS_ALT)
